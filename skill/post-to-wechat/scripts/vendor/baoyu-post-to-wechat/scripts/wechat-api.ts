...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--type" && argv[i + 1]) {
      ...
      args.dryRun = true;
    } else if (arg === "--need-open-comment" && argv[i + 1]) {
      args.needOpenComment = Number(argv[++i]);
    } else if (arg === "--only-fans-can-comment" && argv[i + 1]) {
      args.onlyFansCanComment = Number(argv[++i]);
...
async function main(): Promise<void> {
  ...
  const noc = args.needOpenComment ?? resolved.need_open_comment ?? 1;
  const ofcc = args.onlyFansCanComment ?? resolved.only_fans_can_comment ?? 0;

  console.error(`[wechat-api] Comment flags: need_open_comment=${noc}, only_fans_can_comment=${ofcc}`);
  ...
  const rawCoverPath = args.cover ||
    ...
    frontmatter.cover ||
    frontmatter.image;
  const coverPath = rawCoverPath && !/^https?:\/\//i.test(rawCoverPath) && !path.isAbsolute(rawCoverPath) && args.cover
