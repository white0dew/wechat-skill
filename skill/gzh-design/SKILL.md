...
进入第 0 步前，检查 `{baseDir}/.author-profile.json`。文件存在时**必须读取**，并按 [references/author-profile.md](references/author-profile.md) 处理；仅当它是合法 JSON，且字符串字段 `name`、`bio` 去除首尾空白后都非空时，才是有效的私有默认档案。

署名来源的优先级严格为：**文章中已有的明确署名或用户在本次请求中提供的署名 > 有效的私有默认档案 > 不输出署名**。文章中已有的署名必须保留，不得再新增重复署名。若档案不存在或无效，删除整个作者介绍/签名区域，只保留所选主题的文末 CTA；JSON 无效不得中断排版，交付时说明已忽略无效档案并省略签名区域。任何情况下都不得输出 `{{作者名}}`、`{{简介}}` 或其他作者占位符，也不得推测作者信息。私有 `.author-profile.json` **绝对不得提交到版本库**。
...
6. **开头引言卡署名**：仅使用前置步骤确定的署名来源。文章已有引言署名就原样保留；否则仅在用户本次明确要求把署名放入引言卡时添加。没有明确署名时删除署名行，不得用文章主题充当落款。
7. **尾部作者签名区（仅末尾一处）**：按前置步骤的优先级处理，且作者介绍/签名区域最多出现一次。
   - 文章已有明确签名（如“我是 XXX……”）时原样保留，不再根据请求或默认档案新增另一处签名；若需并入主题尾部区域，只移动原文，不改写作者信息。
   - 用户在本次请求中提供署名时，使用其明确提供的内容；缺少 `name` 或 `bio` 时不得补写、猜测或放置占位符。
   - 文章和本次请求均无署名时，才可用有效私有默认档案生成作者介绍：`我是 {name}，{bio}`。
   - 没有有效署名来源时，删除主题骨架中的整个作者介绍/签名区域，仅保留主题 footer CTA。即使主题组件含作者占位符，也不得把占位符带入产物。
   - 主题 footer CTA 的互动引导可保留：`如果你觉得今天这篇有收获，欢迎**点赞、在看、转发**三连，我们下篇见`。
...
- **签名区最多一个**：按前置步骤确定署名来源；原文已有作者签名时保留且不重复。没有有效署名来源时不生成签名区，只保留主题 CTA；原文已有“点赞在看转发三连”类段落时并入唯一的 CTA 卡片，不要重复生成。
